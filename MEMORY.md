...
- Regex preparation fallback was removed in 1.2.0; AST/token-stream preparation errors now propagate to the caller.
...
- AST parsing now enables Flexmark table extension; GFM-style table delimiters/alignment are preserved in AST translation flow.
...
- Whitespace preservation around protected token boundaries is configurable via `MarkdownTranslationOptions.setPreserveWhitespaceAroundProtectedSegments(...)` (default true) to support languages like Chinese/Japanese.
...
- Public API now includes `explainMarkdown(String)` returning `ExplainMarkdownResult` (processing mode + tokens/chunks) for fast debugging without calling ML Kit translation engine.
